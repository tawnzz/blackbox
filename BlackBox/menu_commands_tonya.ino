...
//  Distance
  ...
  if (distance >= 100) {
    ...
    int curr_distance =  analogRead(IR_sensor);
    if (curr_distance > prev_distance) {
      sfx.volUp();
      Serial.println("Insert Coin to Disengage");
      sfx.playTrack((uint8_t)(6));
      Serial.println("Take out your wallet right now!");
      sfx.playTrack((uint8_t)(5));
    } else if (curr_distance < prev_distance) {
      sfx.volDown();
      Serial.println("I know you don't like this sound");
      sfx.playTrack((uint8_t)(8));
      } else {
        sfx.playTrack((uint8_t)(4));
      }
   }
